...
class NFW(Potential_Base):
    ...
    The formula is:  
    vec(a) = -G*M_vir*[ (a+r)*ln(1+r/a)-r ]/[ r**3*(a+r)*delta_c ]*vec(r), 
    where delta_c = ln(1+c)-c/(1+c)
    pot = -G*M_vir*ln(1+r/a)/(r*delta_c)

    The typical values for the constants for the MW galaxy are (from the slides):
    M_vir = 8e11 Msun
    a = 16 kpc
    c = 15.3
    """
    ...
    def _acceleration(self, particles: Particles, softening: float) \
        -> Tuple[npt.NDArray[np.float64],Optional[npt.NDArray[np.float64]],Optional[npt.NDArray[np.float64]]]:
...
class myamoto_nagai_disc(Potential_Base):
    ...
    ax = -G*M_d* x/D**(3/2)
    ay = -G*M_d* y/D**(3/2)
    az = -G*M_d* [ a + np.sqrt(b**2+z**2) ]/[ np.sqrt(b**2+z**2)*D**(3/2) 
    D = R**2 + ( a+np.sqrt(z**2+b**2) )**2 
    pot = -G*M_d/np.sqrt(D)
    ...
    def __init__(self, Md: float, a: float, b:float):
    ...
    def _acceleration(self, particles: Particles, softening: float) \
        ...
        R = np.sqrt(particles.pos[:,0]**2 + particles.pos[:,1]**2) # R = sqrt(x^2 + y^2), cylindrical radius
        z = particles.pos[:,2]
        D = R**2 + ( self.a + np.sqrt(z**2 + self.b**2) )**2

        ax = -self.Md * particles.pos[:,0] / D**(3/2)
        ay = -self.Md * particles.pos[:,1] / D**(3/2)
        az = -self.Md * ( self.a + np.sqrt(self.b**2 + z**2) ) / ( np.sqrt(self.b**2 + z**2) * D**(3/2) )
...
class Hernquist_bulge(Potential_Base):
    ...
    def __init__(self, Ms: float, c: float):
    ...
    def _acceleration(self, particles: Particles, softening: float) \
        ...
        r = particles.radius()
        acc = -self.Ms/(r*(self.c+r)**2) * particles.pos

        pot = -self.Ms/(self.c+r)

        return acc, None, pot
    


### GIULIANO'S IMPLEMENTATION
